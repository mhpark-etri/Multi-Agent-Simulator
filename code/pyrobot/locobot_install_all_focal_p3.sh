...
apt-get update
apt-get install lsb sudo vim wget curl -y
...
if [ $ROS_NAME == "kinetic" ]; then
	...
	if [ $(dpkg-query -W -f='${Status}' ros-melodic-desktop-full 2>/dev/null | grep -c "ok installed") -eq 0 ]; then 
        ...
        if [ $(dpkg-query -W -f='${Status}' ros-noetic-desktop-full 2>/dev/null | grep -c "ok installed") -eq 0 ]; then
                echo "Installing ROS..."
#               sudo sh -c 'echo "deb http://packages.ros.org/ros/ubuntu $(lsb_relese -sc) main" > /etc/apt/sources.list.d/ros1-latest.list'
                ...
                echo "ros-noetic-desktop-full is already installed";
        fi
fi
...
if [ ! -d "$LOCOBOT_FOLDER/src/pyrobot/robots/LoCoBot/thirdparty" ]; then
  	...
  	cd thirdparty
		...
		git clone -b noetic-devel https://github.com/machinekoder/ar_track_alvar.git
	...
	if [ $ROS_NAME == "kinetic" ]; then
		...
		#cd ORB_SLAM2 && git checkout ec8d750d3fc813fe5cef82f16d5cc11ddfc7bb3d && cd ..
		cp /mnt/temp2/System.cc $LOCOBOT_FOLDER/src/pyrobot/robots/LoCoBot/thirdparty/ORB_SLAM2/src/
		cp /mnt/temp2/System.h $LOCOBOT_FOLDER/src/pyrobot/robots/LoCoBot/thirdparty/ORB_SLAM2/include/
...
cp /mnt/temp2/install_orb_slam2.sh $LOCOBOT_FOLDER/src/pyrobot/robots/LoCoBot/install
cp /mnt/temp2/CMakeLists_orb_slam2_ros.txt $LOCOBOT_FOLDER/src/pyrobot/robots/LoCoBot/locobot_navigation/orb_slam2_ros/CMakeLists.txt
...
if [ ! -d "$LOCOBOT_FOLDER/src/turtlebot" ]; then
	...
	cd kobuki && git checkout ${ROS_NAME1} && cd ..
...
#	sudo apt-get install ros-${ROS_NAME1}-kobuki-* -y
#	sudo apt-get install ros-${ROS_NAME1}-ecl-streams -y
fi
...
if [ $PYTHON_VERSION == "3" ]; then
        cp /mnt/temp2/CMakeLists_orb_slam2_ros.txt $LOCOBOT_FOLDER/src/pyrobot/robots/LoCoBot/locobot_navigation/orb_slam2_ros/CMakeLists.txt
	cp /mnt/temp2/CMakeLists_ar_track_alvar.txt $LOCOBOT_FOLDER/src/pyrobot/robots/LoCoBot/thirdparty/ar_track_alvar/ar_track_alvar/CMakeLists.txt
	cd $LOCOBOT_FOLDER
	source /opt/ros/$ROS_NAME/setup.bash
  	if [ $INSTALL_TYPE == "full" ]; then
    	        source $CAMERA_FOLDER/devel/setup.bash
	...
	echo "source $LOCOBOT_FOLDER/devel/setup.bash" >> ~/.bashrc
	source $LOCOBOT_FOLDER/devel/setup.bash
	echo "End of catkin_make------------------------"
	#read kang
	
	cp ~/install_pyrobot.sh $LOCOBOT_FOLDER/src/pyrobot
	cd $LOCOBOT_FOLDER/src/pyrobot
	chmod +x install_pyrobot.sh
	...
	cd $LOCOBOT_FOLDER/src/pyrobot/robots/LoCoBot
	cp /mnt/temp2/requirements_python3.txt $LOCOBOT_FOLDER/src/pyrobot/robots/LoCoBot/requirements_python3.txt
	source ~/${virtualenv_name}/bin/activate
	echo "Start of install requirement 4 pyrobot------------------------"
	echo $PWD
	#read kang
