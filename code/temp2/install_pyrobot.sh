...
#sudo apt-get -y install ros-${ROS_DISTRO}-orocos-kdl ros-${ROS_DISTRO}-kdl-parser-py ros-${ROS_DISTRO}-python-orocos-kdl ros-${ROS_DISTRO}-trac-ik
...
#		sudo apt-get -y install python-catkin-tools python3.6-dev python3-catkin-pkg-modules python3-numpy python3-yaml
		sudo apt-get -y install python3-catkin-tools python3.8-dev python3-catkin-pkg-modules python3-numpy python3-yaml
...
#		virtualenv -p /usr/bin/python3.6 $VIRTUALENV_FOLDER
		virtualenv -p /usr/bin/python3.8 $VIRTUALENV_FOLDER
		source ~/${virtualenv_name}/bin/activate
		pip install catkin_pkg pyyaml empy rospkg
		python -m pip install --upgrade numpy
		cp /mnt/temp2/requirements_python3_1.txt ./requirements_python3.txt
		pip install .
		deactivate
	fi
	...
	if [ ! -d "$PYROBOT_PYTHON3_WS/src" ]; then
		...
		if [ $ROS_NAME == "kinetic" ]; then
			...
			git clone -b noetic https://github.com/kalyanvasudev/vision_opencv.git
		#	git clone -b python3_patch_noetic https://github.com/kalyanvasudev/vision_opencv.git
...
#		sudo apt-get -y install ros-$ROS_NAME-cv-bridge
		sudo apt-get -y install ros-$ROS_DISTRO-cv-bridge
...
#		catkin_make --cmake-args -DPYTHON_EXECUTABLE=$(which python) -DPYTHON_INCLUDE_DIR=/usr/include/python3.6m -DPYTHON_LIBRARY=/usr/lib/x86_64-linux-gnu/libpython3.6m.so
		echo "------------------ catkin_make starting ------------"
		#read kang
		cp /mnt/temp2/CMakeLists_roscpp.txt /root/pyrobot_catkin_ws/src/ros_comm/clients/roscpp/CMakeLists.txt
		cp /mnt/temp2/CMakeLists_message_filters.txt /root/pyrobot_catkin_ws/src/ros_comm/utilities/message_filters/CMakeLists.txt
		cp /mnt/temp2/CMakeLists_test_roscpp.txt /root/pyrobot_catkin_ws/src/ros_comm/test/test_roscpp/CMakeLists.txt
		cp /mnt/temp2/CMakeLists_geometry2_tf2.txt /root/pyrobot_catkin_ws/src/geometry2/tf2/CMakeLists.txt
