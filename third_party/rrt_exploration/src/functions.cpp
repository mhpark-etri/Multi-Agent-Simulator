...
int indx=(int)(floor((Xp[1]-Xstarty)/resolution)*width + floor((Xp[0]-Xstartx)/resolution));
if (indx < 0 || indx >= (int)Data.size()) {
  return -1;  // outside published map → treat as unknown, not free
}
